                                            ...
                                            <form id="filterForm" class="row g-3">
                                                <div class="col-md-4">
                                                    <select class="form-select" aria-label=".form-select-sm example"
                                                        ...
                                                        <option value=""
                                                            {{ request()->get('status') == '' ? 'selected' : '' }}>Trạng
                                                            thái</option>
                                                        <option value="1"
                ...
                switch (coupon.applies_to) {
                    case 'product':
                        if (coupon.products && coupon.products.length > 0) {
                            var productNames = coupon.products.map(function(product) {
                                return product.name;
                            }).join(', ');
                            relatedValue = productNames || 'Không có sản phẩm nào';
                        } else {
                            relatedValue = 'Không có sản phẩm nào';
                        }
                        break;

                    case 'user':
                        if (coupon.users && coupon.users.length > 0) {
                            var userNames = coupon.users.map(function(user) {
                                return user.username;
                            }).join(', ');
                            relatedValue = userNames || 'Không có người dùng nào';
                        } else {
                            relatedValue = 'Không có người dùng nào';
                        }
                        break;

                    case 'category':
                        if (coupon.categories && coupon.categories.length > 0) {
                            var categoryNames = coupon.categories.map(function(category) {
                                return category.name;
                            }).join(', ');
                            relatedValue = categoryNames || 'Không có danh mục nào';
                        } else {
                            relatedValue = 'Không có danh mục nào';
                        }
                        break;

                    case 'all':
                        relatedValue = 'Áp dụng toàn bộ cửa hàng'; // Hiển thị thông báo cho trường hợp 'all'
                        break;

                    default:
                        relatedValue = 'Không áp dụng';
                }

                $('#RelatedInfo').text(relatedValue);
